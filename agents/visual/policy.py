...
class ActorCritic(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, policy_input, greedy=False):
        self.eval()
    ...
    @torch.no_grad()
    def sample_with_log_prob(self, policy_input, temperature=1.0):
        self.eval()
    ...
    def sample_as_env_action(self, policy_input, greedy=False) -> np.ndarray:
        action = self.sample(policy_input, greedy)
        return action.detach().cpu().numpy()

    def update(self, policy_input, action, old_log_prob, reward, termination, logger=None):
        """
        ...
        with torch.autocast(device_type="cuda", dtype=self.amp_tensor_dtype, enabled=self.use_amp):
            logits, raw_value = self.get_logits_raw_value(policy_input)
            ...
            slow_value = self.slow_value(policy_input)
