...
from feature_extractor.cutie.cutie.inference.memory_manager import MemoryManager
from feature_extractor.cutie.cutie.inference.object_manager import ObjectManager
from feature_extractor.cutie.cutie.inference.image_feature_store import ImageFeatureStore
from feature_extractor.cutie.cutie.model.cutie import CUTIE
from feature_extractor.cutie.cutie.utils.tensor_utils import pad_divide_by, unpad, aggregate
...
class InferenceCore:
    ...
    def step(
        ...
        *,
        ...
        if mask is not None:
            ...
            pred_prob_with_bg = aggregate(mask, dim=0)  # eq8 ?
    ...
    def get_aux_outputs(self, image: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        for k, v in aux_outputs.items():
            if len(v.shape) == 5:
                aux_outputs[k] = F.interpolate(v[0], size=image.shape[-2:], mode="bilinear", align_corners=False)
            elif "weights" in k:
                ...
                aux_outputs[k] = F.interpolate(v, size=image.shape[-2:], mode="bilinear", align_corners=False)[0]
            aux_outputs[k] = unpad(aux_outputs[k], pads)
            if "weights" in k:
                weights = aux_outputs[k]
                weights = weights / (weights.max(-1, keepdim=True)[0].max(-2, keepdim=True)[0] + 1e-8)
